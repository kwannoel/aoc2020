...
solve = product . fmap (uncurry solver) . zip paths . repeat
    where
        paths = [ Path {x = 1, y = 1}
                , Path {x = 3, y = 1}
                , Path {x = 5, y = 1}
...
solver :: Path -> Input -> Solution
solver Path{..} Input{..} = foldl' (uncurry <$> f) 0
                          $ zip [pos `mod` rowLength | pos <- [x, 2 * x ..]]
                                (tail $ takeEvery y rows)
    where
        f :: Int -> Int -> Row -> Int
